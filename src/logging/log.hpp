...
namespace logging
{
	...
	static log_manager< file_log_policy > logger_hinst;
	...
	class logger// : public logger_interface
	{
		...
		log_manager< log_policy >* logger_hinst;
	...
	logger< log_policy >::logger(log_manager< log_policy >* log_h, const std::string& filename)
	{
		logger_hinst = log_h;
		log_line_number = 0;
		log_handler = logger_hinst->log_open(filename);
	}
	...
	{
		logger_hinst->print( log_handler, get_logline_header() + log_stream.str() );
		log_stream.str("");
	}
	...
	{
		logger_hinst->log_close(log_handler);
	}
}
